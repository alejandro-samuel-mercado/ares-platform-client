...
export default function CredencialesAdminPage() {
    ...
    return (
        <div style={{ padding: '2rem', maxWidth: '1400px', margin: '0 auto', paddingBottom: '6rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', marginBottom: '3.5rem' }} className='max-sm:flex-col max-sm:items-start max-sm:gap-4'>
                <div className='    '>
                    <h1 style={{ fontSize: '2.8rem', fontWeight: 900, display: 'flex', alignItems: 'center', gap: '1rem' }} className=''>
                        ...
                        <button onClick={load} className="btn-secondary max-xl:-mt-10 max-xl:mr-10 max-sm:-ml-16" style={{ padding: '1rem ', borderRadius: '50%' }}>
                            <RefreshCw size={20} className={loading ? 'animate-spin' : ''} />
                        </button>
                    </h1>
                    <p className='max-sm:hidden' style={{ fontWeight: 800, opacity: 0.5, color: 'var(--text-muted)' }}>BANCO DE CUENTAS Y ACCESOS DEL SISTEMA</p>
                ...
                <button className="btn-primary" onClick={() => { resetForm(); setShowModal(true); }} style={{ padding: '1rem 2rem' }}>
                ...
                <div style={{ flex: 1, minWidth: '300px' }}>
                    <Combobox
                        ...
                        options={[
                            ...
                            ...servicios.map(s => ({ id: s.id, nombre: s.nombre.toUpperCase(), logo_url: s.logo_url }))
                        ]}
                        value={filterServicio}
                        onChange={(val: any) => setFilterServicio(val)}
                    />
                </div>
                <div style={{ width: '220px' }}>
                    <Combobox
                        ...
                        options={[
                            ...
                            { id: 'ASIGNADA', nombre: 'ASIGNADAS' }
                        ]}
                        value={filterDisponible}
                        onChange={(val: any) => setFilterDisponible(val)}
                    />
                </div>
            ...
            {loading ? (
                ...
                <div className="table-container max-xl:w-[115%] max-xl:-ml-15  max-md:-ml-10 max-md:w-[110%] max-sm:w-[124%] max-sm:-ml-10">
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                                <div>
                                    <Combobox
                                        ...
                                        options={servicios.map(s => ({ id: s.id, nombre: s.nombre.toUpperCase(), logo_url: s.logo_url }))}
                                        value={form.servicio_id}
                                        onChange={(val: any) => setForm({ ...form, servicio_id: val })}
                                    />
                                </div>
                            ...
                            <div style={{ marginBottom: '2.5rem' }}>
                                <Combobox
                                    ...
                                    options={vendors.map(v => ({
                                        ...
                                        subtext: `@${v.alias.toUpperCase()}`
                                    }))}
                                    value={assignVendorId}
                                    onChange={(val: any) => setAssignVendorId(val)}
                                />
