      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3.5rem' }}>
        <div>
          <h1 style={{ fontSize: '2.5rem', display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            CONTROL DE <span className="text-gradient-primary">PAGOS</span>
            <button 
                onClick={loadData} 
              ...
              <div className="modal-overlay" onClick={() => setShowManualModal(false)}>
                  ...
                  >
                        ...
                        <form onSubmit={handleRegisterManual} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                            <div>
                                <label className="input-label">VENDEDOR / CLIENTE</label>
                                <select 
                                    className="input" 
                                    value={manualPago.vendor_id} 
                                    onChange={e => setManualPago({...manualPago, vendor_id: e.target.value})}
                                    required
                                >
                                    <option value="">Seleccione un vendedor...</option>
                                    {vendors.filter(v => v.alias !== 'admin').map(v => (
                            ...
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                                <div>
                                    <label className="input-label">PLAN A ACTIVAR</label>
                                    <select 
                                        className="input" 
                                        value={manualPago.plan_id}
                                        onChange={e => {
                                            const plan = plans.find(p => p.id === e.target.value);
                                            setManualPago({...manualPago, plan_id: e.target.value, monto: plan?.precio || 0});
                                        }}
                                        required
                                    >
