...
export default function ProviderCredencialesPage() {
    ...
    return (
        <div style={{ padding: '1.5rem 1.5rem 8rem 1.5rem' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3.5rem' }} className="max-md:flex-col max-md:gap-6 max-md:flex-col max-md:gap-6">
                <div>
                    <h1 style={{ fontSize: '2.8rem', fontWeight: 900, display: 'flex', alignItems: 'center', gap: '1rem' }} className='max-sm:flex-col'>
                        GESTIÓN <span className="text-gradient-primary">INVENTARIO</span>
                        <button onClick={fetchData} className="btn-secondary" style={{ padding: '0.75rem', borderRadius: '50%' }}>
                            <RefreshCw size={24} className={loading ? 'animate-spin' : ''} />
                        </button>
                    </h1>
                    <p style={{ fontWeight: 800, opacity: 0.5, color: 'var(--text-muted)' }} className='max-sm:hidden'>BANCO DE CUENTAS DE TUS SERVICIOS PROPIOS</p>
            ...
            {loading ? (
                ...
                <div className="table-container max-md:-ml-12 max-md:w-[113%]   max-sm:-ml-10 max-sm:w-[130%]">
