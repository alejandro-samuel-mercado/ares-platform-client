    ...
    return (
        <div style={{ maxWidth: '1400px', margin: '0 auto', padding: '1.5rem' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '3.5rem' }}>
                <div>
                    <h1 style={{ fontSize: '2.8rem', fontWeight: 900, display: 'flex', alignItems: 'center', gap: '1rem' }}>
                        GESTIÓN DE <span className="text-gradient-primary">PROVEEDOR</span>
                        <button onClick={loadData} className="btn-secondary" style={{ padding: '0.75rem', borderRadius: '50%' }}>
                            <RefreshCw size={24} className={loading ? 'animate-spin' : ''} />
                        </button>
                    </h1>
                    <p style={{ fontWeight: 800, opacity: 0.5, color: 'var(--text-muted)' }}>PANEL MAESTRO DE SERVICIOS Y ENTREGAS</p>
                </div>
            ...
            <div style={{ display: 'flex', gap: '1.5rem', marginBottom: '3rem', alignItems: 'center' }}>
                ...
                {activeTab === 'PEDIDOS' && (
                    <div style={{ display: 'flex', gap: '0.5rem', background: 'var(--surface-raised)', padding: '0.4rem', borderRadius: '16px', border: '2px solid #000' }}>
                        {['ALL', 'PENDIENTE', 'EN_PROCESO', 'COMPLETADO'].map(st => (
                            <button
                                ...
                                style={{
                                    ...
                                    color: filter === st ? '#000' : 'var(--text-muted)',
                                    transition: '0.2s'
                                }}
                            >
                                {st === 'ALL' ? 'TODO' : st}
                                            ...
                                            <motion.tr key={s.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}>
                                                <td>
                                                    <div style={{ display: 'flex', alignItems: 'center', gap: '1.25rem' }}>
                                                        ...
                                                        <div>
                                                            <div style={{ fontWeight: 900, fontSize: '1rem' }}>{s.nombre}</div>
                                                ...
                                                <td><div className="chip chip-blue">{s._count.clicks} INTERESADOS</div></td>
                                                <td><div className="chip chip-gold">{(s as any).stock} / {(s as any).total_credenciales} DISPONIBLES</div></td>
                    ...
                    <div className="modal-overlay">
                        <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="modal-container" style={{ maxWidth: '600px' }}>
                            ...
                            <div style={{ marginBottom: '2rem' }}>
                                <label className="input-label">Respuesta Maestra (Credenciales + Instrucciones)</label>
                                <textarea
                                    className="input"
                                    rows={8}
