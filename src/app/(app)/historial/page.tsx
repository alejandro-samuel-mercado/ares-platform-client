        ...
        <div style={{ paddingBottom: '8rem' }}>
            <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} style={{ marginBottom: '2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
                <div className='max-sm:w-full'>
                    <h1 style={{ fontSize: '2rem', fontWeight: 900, display: 'flex', alignItems: 'center', gap: '0.75rem' }} className='max-sm:flex-col max-sm:w-full'>
                    ...
                    <p style={{ fontSize: '0.75rem', fontWeight: 800, opacity: 0.5 }} className='max-sm:hidden'>TODOS LOS SERVICIOS QUE HAS ACTIVADO EN TU CUENTA</p>
                </div>
            </motion.div>
            ...
            {/* Filter Tabs */}
            <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '2rem', background: 'var(--ambient-1)', borderRadius: 20, padding: '0.4rem' }}>
                {(['TODOS', 'ACTIVOS', 'INACTIVOS'] as const).map(f => (
                    <button key={f} onClick={() => setFiltro(f)}
                        style={{
                            flex: 1, border: 'none', padding: '0.7rem', borderRadius: 16, fontWeight: 900, fontSize: '0.7rem', cursor: 'pointer', transition: 'all 0.2s',
